...
#------------------------------------------------------------
ip netns exec athos "${P_IP}" ioam gobschema del 77 &>/dev/null || true
ip netns exec athos "${P_IP}" ioam gobschema add 77 object "${BPF_OBJ}" \
    section ioam6_cntv2
ip netns exec athos "${P_IP}" ioam namespace set 123 gobschema 77

#------------------------------------------------------------
# 10. Gob-schema configuration for porthos (section ioam6_cntv3)
#------------------------------------------------------------
ip netns exec porthos "${P_IP}" ioam gobschema del 77 &>/dev/null || true
ip netns exec porthos "${P_IP}" ioam gobschema add 77 object "${BPF_OBJ}" \
    section ioam6_cntv3
ip netns exec porthos "${P_IP}" ioam namespace set 123 gobschema 77

#------------------------------------------------------------
# 11. Gob-schema configuration for aramis (section ioam6_cntv2)
#------------------------------------------------------------
ip netns exec aramis "${P_IP}" ioam gobschema del 77 &>/dev/null || true
ip netns exec aramis "${P_IP}" ioam gobschema add 77 object "${BPF_OBJ}" \
    section ioam6_cntv2
ip netns exec aramis "${P_IP}" ioam namespace set 123 gobschema 77
